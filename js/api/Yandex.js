...
class Yandex {
    ...
    static getUploadedFiles(callback) {
        createRequest({
            ...
            if (error) {
                console.error('Не удалось получить загруженные файлы:', error);
                return callback(error); // Передаем ошибку обратно
            }
            callback(null, data); // В противном случае передаем данные
        });
    }
